...
+33612345678,Hello Paul! Your order #9012 is ready.`}
  ...
  "error": {
    ...
    "details": "One or more phone numbers are invalid"
      ...
      {/* ── SDKs & Libraries ──────────────────────────────────────────── */}
      <section id="sdks" className="mb-8">
        <h2 className="text-2xl font-bold text-foreground mb-4">SDKs & Libraries</h2>
        <p className="text-muted-foreground mb-6">
          Use our official SDKs to integrate MboaSMS into your application even faster.
        </p>
          ...
          {[
            {
              lang: "Node.js",
              icon: "🟢",
              install: "npm install mboasms",
              snippet: `const MboaSMS = require("mboasms");
const client = new MboaSMS("YOUR_TOKEN");
...
await client.send({
  ...
  message: "Hello!",
            ...
            },
            {
              lang: "Python",
              icon: "🐍",
              install: "pip install mboasms",
              snippet: `from mboasms import MboaSMS

client = MboaSMS("YOUR_TOKEN")
...
client.send(
    ...
    message="Hello!",
            ...
            },
            {
              lang: "PHP",
              icon: "🐘",
              install: "composer require mboasms/sdk",
              snippet: `use MboaSMS\\Client;
...
$client->send([
    ...
    "message" => "Hello!",
            ...
            <div key={sdk.lang} className="rounded-xl border border-border bg-card overflow-hidden hover:border-primary/30 transition-colors">
              <div className="px-4 py-3 border-b border-border bg-muted/20 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span>{sdk.icon}</span>
                  <span className="font-semibold text-sm text-foreground">{sdk.lang}</span>
                </div>
              </div>
              <div className="p-4 space-y-3">
                <div>
                  <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1">Install</div>
                  <code className="text-xs font-mono bg-muted/50 px-2 py-1 rounded block text-primary">{sdk.install}</code>
                </div>
                <div>
                  <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1">Quick start</div>
                  <CodeBlock language={sdk.lang === "Node.js" ? "javascript" : sdk.lang.toLowerCase()} code={sdk.snippet} />
                </div>
              </div>
            </div>
          ))}
        ...
        <InfoCard type="info" title="Need help?">
          Our support team is available to help with integration questions.
          Contact us at <a href="mailto:contact@mboasms.com" className="text-primary hover:underline font-medium">contact@mboasms.com</a> or via the dashboard.
        </InfoCard>
      </section>
    </motion.div>
  );
